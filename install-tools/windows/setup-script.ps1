...
$media_options = New-Object ChoiceDescription "Install &media software", "Install media applications"
...
$options = [ChoiceDescription[]]($all_option, $gaming_option, $dev_option, $web_browser_option, $communication_option, $media_options, $hardware_option, $utils_option, $debloat_option, $quit_option)

do {
    # Show prompt and save user's answer to variable
    $response = $host.UI.PromptForChoice($title, $message, $options, 9)
    $finished = $false
    ...
    switch ($response) {
        ...
        5 {
            # Media
            Log "Install media applications"
            Install-Apps $media_apps
            break
        }
        6 {
